...
class MapPodRacing(gym.Env):
    ...
    def step(self, action):
        ...
        if point_to_segment_distance(self.cp_queue[0][0], self.cp_queue[0][1],
            ...
            if self.my_pod.is_moving_forward(self.cp_queue[0][0], self.cp_queue[0][1]):
                reward = 1
            self.timeout -= 1
            if self.timeout <= 0:
                terminated = True
...
class Pod:
    ...
    def is_moving_forward(self, cp_x, cp_y):
        return math.hypot(self.last_position.x - cp_x, self.last_position.y - cp_y) > math.hypot(self.position.x - cp_x,
                                                                                                 self.position.y - cp_y)
