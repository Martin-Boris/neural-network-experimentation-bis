...
from ppo_v3.mad_pod_racing import MapPodRacing
...
if __name__ == "__main__":
    ...
    action_counter = torch.zeros(12, dtype=torch.int32)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if infos and "episode" in infos:
                valid = [i for i, active in enumerate(infos["_episode"]) if active]
                if valid:
                    avg_reward = np.mean([infos["episode"]["r"][i] for i in valid])
                    avg_length = np.mean([infos["episode"]["l"][i] for i in valid])
